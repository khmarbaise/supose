...
public class SuposeCLI {
    ...
    private static void printHelp() {
        ...
        List<?> helpLines = suposeOptions.helpLines(0,
    ...
    private static void runScan(ScanCommand scanCommand) throws SVNException {
        ...
        long blockNumber = ScanSingleRepository.scanFullRepository(
                scanRepository, url, fromRev, indexDirectory, create,
                authManager);

        LOGGER.info("Scanning of revisions done");

        ScanSingleRepository
                .mergeIndexesAndCleanUp(indexDirectory, blockNumber);
    }

    private static void runSchedule(ScheduleCommand scheduleCommand) {
        String configurationFile = scheduleCommand
                .getConfiguration(commandLine);
        String configurationBaseDir = scheduleCommand
                .getConfBaseDir(commandLine);
        ...
        try {
            ...
            ConfigurationRepositories confRepos = new ConfigurationRepositories(
                    configurationFile);
            LOGGER.info("We have " + confRepos.getNames().length
                    + " repositories.");
            for (int i = 0; i < confRepos.getNames().length; i++) {
                ...
                Repository repository = RepositoryFactory
                        .createInstance(reposConfig);
                if (!repository.validConnection()) {
                    // Connection has failed.
                    System.err.println("The repository " + repository.getUrl()
                            + " can't be connected!");
                    // Do not make a job from a non connectable repository.
                    continue;
                }

                LOGGER.info("The repository " + repositoryName
                        + " is ready for scanning.");
                JobDetail jobDetail = new JobDetail(repositoryName, null,
                        RepositoryScanJob.class);

                jobDetail.getJobDataMap().put(JobDataNames.REPOSITORY,
                        repository);
                jobDetail.getJobDataMap().put(
                        JobDataNames.REPOSITORYCONFIGURATION, reposConfig);
                jobDetail.getJobDataMap().put(JobDataNames.BASEDIR,
                ...
                try {
                    cronTrigger1 = new CronTrigger("SupoSE." + repositoryName,
                            Scheduler.DEFAULT_GROUP, cronExpression);
                } catch (Exception e) {
                    System.err
                            .println("Error for cronTrigger wrong expression for cron: "
                                    + e);
                    System.exit(1);
                }
            ...
            if (scheduledJobs == 0) {
                System.err
                        .println("We couldn't start any scan job so we are dying...");
                scheduler.shutdown();
            }
    ...
    private static void runSearch(SearchCommand searchCommand) {
        ...
        if (xml) {
            ...
            if (result == null) {
                System.out
                        .println("Somethings has gone wrong. Check the log file output!");
                return;
            }
                ...
                for (FieldNames fn : cliDisplayFields) {
                    if (fn.equals(FieldNames.PROPERTIES)) {
                        ...
                        for (Map.Entry<String, String> prop : properties
                                .entrySet()) {
                            System.out.println(" --> K:" + prop.getKey()
                                    + " V:" + prop.getValue());
                        }
                    } else {
                        Object attribute = searchRepository.callGetterByName(
                                item, fn.getValue());
