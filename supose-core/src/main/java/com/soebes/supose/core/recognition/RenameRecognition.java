...
public class RenameRecognition {
    ...
    public ArrayList<RenameType> checkForRename(SVNLogEntry logEntry,
            Set<?> changedPathsSet) {
        ...
        for (Iterator<?> changedPaths = changedPathsSet.iterator(); changedPaths
            ...
            if (entryPath.getType() == SVNLogEntryPath.TYPE_DELETED) {
                ...
                if (possibleRenames.containsKey(deletedEntry)) {
                    ...
                    LOGGER.debug("New Entry for possibleRenames "
                            + deletedEntry);
                ...
                if (entryPath.getCopyPath() != null) {
                    SVNDirEntry destEntry = RepositoryInformation
                            .getInformationAboutEntry(getRepository(),
                                    logEntry.getRevision(), entryPath.getPath());
                    SVNDirEntry sourceEntry = RepositoryInformation
                            .getInformationAboutEntry(getRepository(),
                                    entryPath.getCopyRevision(),
                                    entryPath.getCopyPath());

                    // Source and destination of the copy operation must be
                    // identical
                    ...
                    if (destEntry.getKind() == sourceEntry.getKind()) {
                        ...
                        LOGGER.debug("COPY src:" + sourceName + " dest:"
                                + destinationName);
                        RenameType rt = new RenameType();
                        ...
                        if (possibleRenames.containsKey(sourceName)) {
                            ...
                            // create new entry in the list of possible
                            // candidates.
