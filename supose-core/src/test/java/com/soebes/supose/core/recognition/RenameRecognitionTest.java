...
import com.soebes.supose.core.recognition.RenameRecognition;
import com.soebes.supose.core.recognition.RenameType;
import com.soebes.supose.core.repository.Repository;

public class RenameRecognitionTest extends TestBase {
    private static Logger LOGGER = Logger
            .getLogger(RenameRecognitionTest.class);
    ...
    public void analyzeTestFirstTag() throws SVNException {
        ...
        LOGGER.debug("Renamed: " + rt.getRevision() + " "
                + rt.getDestinationName() + " (from " + rt.getSourceName()
                + ":" + rt.getCopyFromRevision() + ")");
        assertEquals(result.get(0).getSourceName(), "/project1/trunk/f1.txt");
        assertEquals(result.get(0).getDestinationName(),
                "/project1/trunk/f3.txt");
        assertEquals(result.get(0).getCopyFromRevision(), 9);
        assertEquals(result.get(0).getRevision(), 11);
    }

    private ArrayList<RenameType> analyzeLog(Repository repository)
            throws SVNException {
        ...
        for (Iterator<?> iterator = logEntries.iterator(); iterator.hasNext();) {
            ...
            // Only if the changeset contains more than one entry we could have
            // a rename operation.
            if (logEntry.getChangedPaths().size() > 1) {
                Set<?> changedPathsSet = logEntry.getChangedPaths().keySet();

                ArrayList<RenameType> tmpResult = tbr.checkForRename(logEntry,
                        changedPathsSet);
                if (tmpResult.size() > 0) {
